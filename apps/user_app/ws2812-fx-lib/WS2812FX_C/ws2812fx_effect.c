...
// 连续呼吸
uint16_t WS2812FX_mode_breath_continue(void)
{
    // 如果速度值为1，时间基准为1ms，当前一个呼吸周期约为 3000 ms
    ...
    Adafruit_NeoPixel_fill(color, _seg->start, _seg_len);
    // printf("lum: %d, delya : %u \r", lum, delay);
    ...
    // return delay + (u32)delay * 9 * 44466 / 100000;
    // return delay + (u32)delay * (9 * 1.1100 ); // 14.2s
    // return delay + (u32)delay * (9 * 1.1900 ); // 16.2s
    // return delay + (u32)delay * (9 * 1.1600 ); // 15.9s
    // return delay + (u32)delay * (9 * 1.1555 ); // 15.89s
    // return delay + (u32)delay * (9 * 1.1450 ); // 15.7s
    // return delay + (u32)delay * (9 * 1.1300 ); // 15.6
    // return delay + (u32)delay * (9 * 1.1150 ); // 15.627
    // return delay + (u32)((double)delay * 9 * 1.2 ); //
    // return delay + (u32)(delay * 9 * 112 / 100); // 15.6
    // return delay + (u32)(delay * 9 * 111 / 100); // 14.3

    // return delay + (u32)(delay * 0 * 112 / 100); // 3s
    // return delay + (u32)(delay * 1 * 112 / 100); // 3.92s 

    // speed范围：0~9，0快，9慢
    return delay + (u32)(delay * _seg->speed * 112 / 100); // 3s ~ 15.6s
}

// 带停顿的呼吸
uint16_t WS2812FX_mode_breath_with_pause(void)
{
    // 如果速度值为1，时间基准为1ms，当前一个呼吸周期约为 5000 ms（不包括lum从0~15的变化部分）
    // lum在0~15时，灯灭，持续 5000 ms，再从15变到255
    int lum = _seg_rt->counter_mode_step;
    ...
    {
        step = 0;
        lum = _seg_rt->counter_mode_step;
    }
    else if (_seg_rt->counter_mode_step <= 511)
    {
        step = 1;
        lum = 511 - _seg_rt->counter_mode_step;
    }
    else
    {
        step = 2;
    ...
    if (lum <= 25)
        ...
        delay = delay + (u32)delay * 12 * _seg->speed / 10 / 9;
    } 
    else
    {
        delay = 5000 + 5000 * _seg->speed / 9; // 5000 ~ 10000
        // return delay;
    }
    ...
    {
        color = WS2812FX_color_blend(_seg->colors[1], _seg->colors[0], lum);
    }
    Adafruit_NeoPixel_fill(color, _seg->start, _seg_len);
    // printf("lum: %d, delya : %u \r", lum, delay);

    _seg_rt->counter_mode_step += 2;
    if (_seg_rt->counter_mode_step > (512))
    {
        _seg_rt->counter_mode_step = 0;
        SET_CYCLE;
    }
    ...
    // return delay;
    // return delay + (u32)(delay % 10); //
    // return delay + ((u32)delay * 10 / 10); // 18.3s
    // return delay + ((u32)delay * 1); // 18.3s
    // return delay + ((u32)delay * 11 / 10); // 18.9
    // return delay + ((u32)delay * 12 / 10); // 21.1s
    // return delay + ((u32)delay * 15 / 10); // 23.5s
    // return delay + ((u32)delay * 2); // 27.7s

    // return delay + ((u32)delay * 12 / 10) * 9 / 9; // 21.1s
    // return delay + ((u32)delay * 12 / 10) * 1 / 9; //  10.9s
    // return delay + ((u32)delay * 12 / 10) * 0 / 9; // 10.0
    
    // speed范围：0~9，0快，9慢
    // return delay + (u32)delay * 12 * _seg->speed / 10 / 9;
    return delay;
}

uint16_t WS2812FX_mode_fire_flicker_ctl_by_speed(void)
{
    return WS2812FX_fire_flicker(_seg->speed);
}
